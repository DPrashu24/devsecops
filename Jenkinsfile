  ...
  tools { 
   ...
   stages{
	...
	stage('Push') {
            steps {
                script{
                    docker.withRegistry('https://109490194429.dkr.ecr.us-east-1.amazonaws.com/asg', 'ecr:us-east-1:aws-credentials') {
                    app.push("latest")                                                                        
                    }
                }
            }
      ...
      sh 'aws eks update-kubeconfig --region us-east-1 --name kubernetes-cluster'
